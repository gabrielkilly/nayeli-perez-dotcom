...
import YarngdingsIcon from "@/components/YarndingsIcon";
                    ...
                    <Section backgroundColor={Color.Neutral_2}>
                        <div className="flex flex-row w-full justify-between items-center">
                            <div className="flex flex-col space-y-10 w-1/2">
                                <h1 className={`${fontIbmPlexSerif.className} text-5xl lead ng-10 italic`}>{content.welcomeSection.welcomeTitle}</h1>
                                <p className={`${fontWorkSans.className} text-2xl`}>{content.welcomeSection.welcomeText}
                                    <YarngdingsIcon color={Color.Icon_Plum} icon={content.welcomeSection.welcomeIcon} className="px-2" /></p>
                                <div className="flex flex-row space-x-10">
                                    <ButtonLink {...content.welcomeSection.cta1} />
                                    <ButtonLink {...content.welcomeSection.cta2} />
                                </div>
                            </div>
                            <img className="w-full w-1/2" src={content.welcomeSection.welcomeImagePath} alt="Nayeli" />
                        </div>
                    </Section>
...
function SkillItem({ skill } : SkillItemProps) {
    return (
        <div className="flex flex-col space-y-4 px-8">
            <h3>
                <YarngdingsIcon icon={skill.yanrdingsIcon} color={skill.iconColor} /> &nbsp; 
